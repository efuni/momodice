...
* 桃桃姬是为DND跑团的**自动化**需求而设计，她可以自主完成一系列的触发机制，并且能够承担一部分记录工作。    
* 桃桃姬的指令设计源于便捷，因此没有冗长的指令输入，但这需要建立在你充分理解DnD的游戏机制以及桃桃姬的功能之上。   
...
* 需豁免检定的攻击方式  
某些特殊的能力需要目标进行豁免检定来计算受到的伤害。而不是通过骰出攻击骰来决定是否命中该目标。你可以将使用属性项(第三项)改为STRS,DEXS,CONS,INTS,WISS,CHAS中的任意一项，如此桃桃姬便会得知这是一个需要目标进行豁免的攻击方式。  
同时，第二项如果填写0，则代表豁免成功不会受伤，如果填写1，则代表豁免成功伤害减半。目前会使用人物的主要施法属性来计算DC值，在后续的版本更新中，会更新设定DC的选项。  
> [火球术 1 DEXS 6D8<火焰>]  
...
你可以在命令后加上目标的全称，或者是目标的快捷编号，你可以通过<kbd>.所有角色</kbd>指令查看当前群内所有已创建的生物的快捷编号:    
>*=名称*  
*=名称1,名称2,名称3*  
*=编号*  
*=编号1,编号2,编号3*  
...
同样，如果你是一名邪术师，你在战斗前释放了隐形术，想要使用魔能爆偷袭一个敌人。那么你的第一次攻击极有可能取得优势。例如以下指令:  
