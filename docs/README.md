...
在这些情况下，单次检定无法满足上述特质或效应的需求，你需要进行二次检定。桃桃姬会根据你新加入的参数重新计算你上一次的行动并得出新的结果。    
在进行了一次检定后，使用<kbd>..l 参数1 参数2 ...</kbd>或<kbd>..L 参数1 参数2 ...</kbd>这样的形式的指令进行二次检定。  
需要注意的是，参数是可选的，你可以不填写任何参数。另外，该功能目前只支持攻击与伤害检定。  
尝试理解以下指令段：
>..a 斧 劣势  
桃桃姬:"...未命中..."
..l 激励  
...
使用<kbd>.结束战斗</kbd>或<kbd>.结算</kbd>手动切换至本局游戏中某个你创建的角色。  
